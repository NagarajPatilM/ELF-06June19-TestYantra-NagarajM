...
class SearchNavabar extends Component {
    ...
    search() {
        ...
        Axios.get('http://localhost:9002/getUserByName?search=' + this.state.search).then((response) => {
    ...
    logout(e) {
        e.preventDefault();
        Axios.get('http://localhost:9002/logout', null
        ).then((response) => {
            localStorage.clear();
            console.log('Response Object', response.data);
            if (response.data.message === "Success") {
                ...
                <Navbar bg="light" expand="lg">
                    ...
                    <Navbar.Collapse id="basic-navbar-nav">
                        ...
                        <Nav className="nav-link">
                            <NavLink  id="b" className="nav-link" to="/CreateTask">Create Task</NavLink>
                            <NavDropdown title={<Image id="a" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS5TcWEWJgM4M34bRynJ95ezoa16mApLKNMvDcdr9y6cB7Z9ncJVw" roundedCircle />} id="basic-nav-dropdown">
